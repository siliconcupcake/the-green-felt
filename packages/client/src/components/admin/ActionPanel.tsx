...
export function ActionPanel() {
  ...
  return (
    <div className="p-3 border-t border-admin-border-subtle shrink-0 bg-admin-bg-surface">
      <div className="mb-2">
        <strong className="text-admin-text-muted">Acting as:</strong>{' '}
        {activePlayer ? (
          <span className="text-admin-text">
            {activePlayer} ({gameInfo?.players.find((p) => p.id === activePlayer)?.name ?? 'unknown'})
          </span>
        ) : (
          <em className="text-admin-text-dim">No active player</em>
        )}
      </div>
      ...
      <div className="mb-3">
        ...
        {suggestedMove && (
          <div className="mt-2 flex flex-col gap-1.5">
            <textarea
              className="w-full p-1.5 border border-admin-input-border bg-admin-input-bg text-admin-text rounded-badge font-[inherit] text-xs resize-y transition-colors duration-150 focus:border-admin-blue/50 focus:outline-none"
              value={actionJson}
              onChange={(e) => setActionJson(e.target.value)}
              rows={4}
            />
            <button
              className="px-2.5 py-1 border border-admin-green/30 bg-admin-btn-success text-admin-green cursor-pointer rounded-badge font-[inherit] text-xs transition-all duration-150 hover:enabled:bg-admin-btn-success-hover active:enabled:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div className="mb-3">
        <div className="flex items-center gap-2 mb-1">
          <strong className="text-admin-text-muted">Manual:</strong>
